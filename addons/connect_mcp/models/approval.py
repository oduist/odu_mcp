...
class ConnectMcpApproval(models.Model):
    _name = "connect.mcp.approval"
    ...
    payload_json = fields.Text(required=True, readonly=True, groups="connect_mcp.group_mcp_manager")
    ...
    def _system_write(self, values):
        result = super(ConnectMcpApproval, self.with_context(mcp_approval_system_write=True)).write(
            values
        )
        if "state" in values:
            for approval in self:
    ...
    def _gc_approvals(self):
        ...
        retention_days = int(
            self.env["ir.config_parameter"].sudo().get_param(
                "connect_mcp.approval_retention_days", "30"
            )
        )
