...
def _env(name: str, default: str = "") -> str:
    return os.getenv(f"CONNECT_MCP_{name}", default).strip()
...
def _int(name: str, default: int, *, minimum: int, maximum: int) -> int:
    ...
    try:
        ...
        raise ConfigurationError(f"CONNECT_MCP_{name} must be an integer.") from exc
    if not minimum <= parsed <= maximum:
        raise ConfigurationError(f"CONNECT_MCP_{name} must be between {minimum} and {maximum}.")
    return parsed
...
def _float(name: str, default: float, *, minimum: float, maximum: float) -> float:
    ...
    try:
        ...
        raise ConfigurationError(f"CONNECT_MCP_{name} must be numeric.") from exc
    if not minimum <= parsed <= maximum:
        raise ConfigurationError(f"CONNECT_MCP_{name} must be between {minimum} and {maximum}.")
    return parsed
