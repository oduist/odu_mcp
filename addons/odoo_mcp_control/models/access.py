...
class OdooMcpAccess(models.Model):
    _name = "odoo.mcp.access"
    ...
    event_channel = fields.Char(
        ...
        readonly=True,
        copy=False,
        default=lambda self: f"odoo_mcp_{uuid.uuid4().hex}",
        groups="odoo_mcp_control.group_mcp_manager",
    )
    ...
    def init(self):
        self.env.cr.execute(
            """
            SELECT to_regclass('odoo_mcp_credential') IS NOT NULL
                OR EXISTS (
                    SELECT 1 FROM ir_model WHERE model = 'odoo.mcp.credential'
                )
            """
        )
        if self.env.cr.fetchone()[0]:
            raise ValidationError(
                _(
                    "Unsupported odoo_mcp_control database detected: the obsolete "
                    "odoo_mcp_credential table exists. Install this version on a fresh database."
                )
            )
    ...
    def _check_quota(self):
        ...
        audit_model = self.env["odoo.mcp.audit.log"].sudo()
